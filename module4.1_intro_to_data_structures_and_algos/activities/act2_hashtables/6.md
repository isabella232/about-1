...
The **open addressing** method has all the hash keys stored in a fixed length table. With this method a hash collision is resolved by **probing**, or searching through alternate locations in the array \(the _probe sequence_\) until either the target record is found, or an unused array slot is found, which indicates that there is no such key in the table.
...
Below are the basic process of inserting a new key \(_k_\) using open addressing:

1. Compute the position in the table where _k_ should be stored.
2. If the position is empty or deleted, store _k_ in that position.
...
A variation of the linear probing idea is called **quadratic probing**. Instead of using a constant “skip” value, we use a rehash function that increments the hash value by 1, 3, 5, 7, 9, and so on. This means that if the first hash value is _h_, the successive values are ℎ+1, ℎ+4, ℎ+9, ℎ+16, and so on. In other words, quadratic probing uses a skip consisting of successive perfect squares.
...
index = index % hashTableSize index = \(index + 12\) % hashTableSize index = \(index + 22\) % hashTableSize index = \(index + 32\) % hashTableSize

and so on...

![QP](https://slideplayer.com/slide/3869765/13/images/33/Quadratic+Probing+In+quadratic+probing%2C+f+is+a+quadratic+function+of+i%2C+typically+f%28i%29+%3D+i2..jpg)
...
index = \(index + 1  _indexH\) % hashTableSize; index = \(index + 2_  indexH\) % hashTableSize;
